...
class FlutterGoogleStreetView(
    ...
    /** GMS can NPE in streetview.camera if this listener runs before a panorama location exists. */
    private var cameraChangeListenerAttached = false
    init {
    ...
    private fun createInitOption(creationParams: Map<String?, Any?>?): StreetViewPanoramaOptions? =
        if (creationParams != null) StreetViewPanoramaOptions().apply {
            ...
            // Only set an initial panorama camera when Dart sends explicit bearing/tilt/zoom.
            // The upstream plugin always applied (0,0,0) here, which on some Play Services builds
            // triggers a native NPE in Street View camera handling before a panorama location exists.
            if (creationParams.containsKey("zoom") ||
                creationParams.containsKey("tilt") ||
                creationParams.containsKey("bearing")
            ) {
    ...
    private fun removeListener() {
        ...
        cameraChangeListenerAttached = false
    }

    private fun setupListener(streetViewPanorama: StreetViewPanorama) {
        // Intentionally do NOT register OnStreetViewPanoramaCameraChangeListener here.
        // Play Services ~26.x can crash (NPE: location) when internal camera work runs
        // before the first valid StreetViewPanoramaLocation is available.
        cameraChangeListenerAttached = false
        streetViewPanorama.setOnStreetViewPanoramaCameraChangeListener(null)
        streetViewPanorama.setOnStreetViewPanoramaChangeListener(
            PanoListenerBridge.create(this)
        )
    ...
    private fun hasRenderablePanorama(location: StreetViewPanoramaLocation?): Boolean {
        if (location == null) return false
        if (location.position != null) return true
        val pano = location.panoId
        return !pano.isNullOrEmpty()
    }

    private fun maybeAttachCameraChangeListener(location: StreetViewPanoramaLocation?) {
        if (cameraChangeListenerAttached || streetViewPanorama == null) return
        if (!hasRenderablePanorama(location)) return
        cameraChangeListenerAttached = true
...
/*        val info = call.let {
    ...
    /** Called from [PanoListenerBridge] so GMS may pass null without Kotlin non-null bridge NPEs. */
    override fun onStreetViewPanoramaChange(location: StreetViewPanoramaLocation?) {
        maybeAttachCameraChangeListener(location)
